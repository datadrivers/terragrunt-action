...
| `include-workflow-link`          | Include a link to the workflow in the PR comment     | No       | `true`                   |
...
| `hide-previous-comments`         | Hide previous comments generated by this action      | No       | `true`                   |
| `log-changed-resources`          | Log changed resources in the PR comment              | No       | `false`                  |
...
- name: Comment Terraform Plan on PR
  ...
  with:
    ...
    pr-commenter-comment-header: "📋 Terraform Plan Changes"
...
2. **Binary Detection**: Automatically detects whether to use `terraform`, `tofu`, or `terragrunt` (if enabled). Existing `TG_TF_PATH` takes precedence; otherwise `.opentofu-version`, `.terraform-version`, and `.tool-versions` are checked before falling back to an available binary.
