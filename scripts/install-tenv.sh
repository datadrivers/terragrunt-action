...
verify_openssl() {
  ...
  decode_base64 "${sig_path}" "${decoded_sig}"
  decode_base64 "${pem_path}" "${decoded_cert}"
...
download_file "${TAR_FILENAME}"
download_file "${TAR_FILENAME}.sig"
download_file "${TAR_FILENAME}.pem"

echo "Verifying signatures with openssl..."

verify_openssl "${CACHE_DIR}/${TAR_FILENAME}" \
               "${CACHE_DIR}/${TAR_FILENAME}.sig" \
               "${CACHE_DIR}/${TAR_FILENAME}.pem"
